...
export default {
	...
	const corsHeaders: Record<string, string> = {
		...
		if (supportedDomains) {
			...
			// Block requests without Origin header when CORS is configured
			// This helps prevent CLI/curl abuse since browsers always send Origin
			if (!origin) {
				return new Response('Origin header required', {
					status: 403,
					headers: corsHeaders,
				});
			}

			// Check if origin matches any allowed domain (exact match or wildcard subdomain)
			const isAllowed = supportedDomains.some(domain => {
				if (domain.startsWith('.')) {
					// Wildcard subdomain matching (e.g., `.example.com` matches `sub.example.com`)
					try {
